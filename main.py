...
def create_db(): # создает таблицы
    ...
    with conn.cursor() as cur:
        for table_name in reversed(list(queries.keys())):       # DROP all tables
            ...
            # cur.execute("DROP TABLE if exists %s ", (table_name, ))             # <<<<<<< why this statement doesn't work???
...
def get_students(course_id): # возвращает студентов определенного курса
    with conn.cursor() as cur:
        ...
        q = "select st.name, st.gpa, st.birth from student as st \
                       join course_student as cs on cs.id_student = st.id where cs.id_course = %s"
...
def add_students(course_id, students): # создает студентов и записывает их на курс
    with conn.cursor() as cur:
        for st in students:
            ...
            cur.execute(
                "INSERT INTO student(name,gpa,birth) VALUES (%s,%s,to_timestamp(%s,'YYYY-MM-DD')) RETURNING id", (name, gpa, birth))
            st_id = cur.fetchone()[0]
            # cur.execute(f"INSERT INTO course_student(id_student, id_course) VALUES ('{st_id}','{course_id}')")
            cur.execute("INSERT INTO course_student(id_student, id_course) VALUES (%s,%s)", (st_id, course_id))
    conn.commit()
...
def get_student(student_id):
    with conn.cursor() as cur:
        ...
        q = "select st.name, st.gpa, st.birth, c.name from student as st \
                        join course_student as cs on cs.id_student = st.id \
                        join course as c on c.id = cs.id_course where st.id = %s"
